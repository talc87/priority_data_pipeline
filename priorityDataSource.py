...
class priorityDataSource:
    # class variables- relevant to all instances of the class
    client = pymongo.MongoClient(config.mongoDbConnStr, socketTimeoutMS=None)
    metadataMongoDB = client[config.metadataDbName]
    configMongoDB = client[config.configDbName]
    configCollection = configMongoDB[config.configCollectionName]
    datatypeMappingCollection = metadataMongoDB[config.datatypeMappingCollectionName]
    eformCollection = metadataMongoDB['eform']
    ...
    def __init__(self,extractionConfig):
        ...
        self.extractionTimestampUTC = datetime.utcnow()
        ...
        self.sqlConnStr = config.sqlConnStr
    ...
    def refreshMeatdata(self)->dict:
        ...
        #fetch the entity and sub-entity metadata from EFORM entity and werite them to eform MongoDB collection
        eformMetadata = self.getEformhMetadata()
    ...
    def getEformhMetadata(self)->list:
        # get the Priority entity list and the field in each entity (using EFORM and sub-entity FCLMN)
        ...
        eformMetadata = self.entityGetRequest(entity,False).json()['value']
        
        priorityDataSource.eformCollection.insert_many(eformMetadata)
        return eformMetadata
    ...
    def getEntitySubforms(self,entityId: str, suffix:bool)->list:
        ...
        logging.debug(f'querying the subforms for datasource= {self.datasourceID} entityId= {entityId}')
        
        # Query to find the entity metadata document
        query = {"_id": self.datasourceID, "entities.EntityID": entityId}
        ...
        logging.debug(f'returnning the value {subforms}')
        return subforms
    ...
    def entityGetRequest(self,entity:dict, incrementalFlag:bool = True):
        '''
        ...
        if entity.get('filterFlag',False):   #check #filterFlag = True if filterFlag key is missong evaluate to False
            ...
            startDate = None
            fieldParam=None
            logging.debug(f'filter flag is set to {entity["filterFlag"]}, the entire table will be truncated and re-writes')
